    ...
    <footer className="bg-navy text-navy-foreground">
      <div className="container-px mx-auto max-w-7xl py-16">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="grid h-9 w-9 place-items-center rounded-md bg-gold/15 text-gold font-serif text-lg">E</span>
              <span className="font-serif text-xl font-semibold">
                Elevare <span className="text-gold">SEO</span>
              </span>
            </div>
            <p className="mt-4 max-w-xs text-sm text-white/65">
              Premium search engine optimization for businesses of every size and industry.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-4 text-sm text-white/70">Request a complimentary audit of your site's search performance.</p>
            <Link
              to="/contact"
        ...
        <div className="mt-12 flex flex-col items-start justify-between gap-2 border-t border-white/10 pt-6 text-xs text-white/50 md:flex-row md:items-center">
          <p>© {new Date().getFullYear()} Elevare SEO. All rights reserved.</p>
          <p>Crafted with precision for measurable growth.</p>
        </div>
      </div>
    </footer>
  );
