    ...
    <div>
      <section className="gradient-ink relative overflow-hidden py-24 md:py-32">
        ...
        <div className="container-px pointer-events-none relative mx-auto max-w-7xl">
          <div className="rise-in"><Eyebrow>About BasedClicksCo</Eyebrow></div>
          <h1 className="rise-in rise-delay-1 mt-5 max-w-3xl font-serif text-5xl md:text-6xl">About <span className="gold-shimmer">BasedClicksCo</span></h1>
          <p className="rise-in rise-delay-2 mt-7 max-w-2xl text-lg leading-relaxed text-foreground/65">We are search engine optimization experts delivering SEO services for small businesses, mid-size companies, and enterprise SEO solutions — transparent, strategic, and genuinely invested in client outcomes.</p>
        </div>
      </section>
        ...
        <div className="container-px mx-auto grid max-w-7xl gap-14 lg:grid-cols-2 lg:items-center">
          <Reveal>
            <div className="frame-glow relative overflow-hidden">
              <img src={aboutImg} alt="BasedClicksCo SEO experts in a strategy session" width={1200} height={900} loading="lazy" className="w-full rounded-2xl object-cover" />
            </div>
          </Reveal>
          <Reveal>
            <Eyebrow>Our story</Eyebrow>
            <h2 className="mt-5 font-serif text-4xl md:text-5xl">SEO experts helping businesses of every size grow online</h2>
            <div className="mt-7 space-y-5 leading-relaxed text-muted-foreground">
              ...
              <p>Today we work with single-location shops, fast-growing startups, and established businesses in Simi Valley, California. From <Link to="/services" className="text-foreground underline decoration-gold/50 underline-offset-4 transition-colors hover:text-gold">technical SEO audits</Link> to content marketing and local SEO services, every engagement is custom — and the standard is always premium.</p>
              <p>Every client gets a senior team, transparent reporting, and a partner committed to helping you increase website traffic and revenue. See our <Link to="/case-studies" className="text-foreground underline decoration-gold/50 underline-offset-4 transition-colors hover:text-gold">case studies</Link> for proof.</p>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="border-y border-white/8 bg-white/[0.02] py-24">
        <div className="container-px mx-auto max-w-7xl">
          <Reveal className="text-center">
            ...
            <h2 className="mt-5 font-serif text-4xl md:text-5xl">Our values</h2>
          </Reveal>
          <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {values.map((v, i) => (
              <Reveal key={v.title} style={{ transitionDelay: `${i * 70}ms` }}>
                <div className="panel panel-hover h-full rounded-xl p-7">
                  <div className="grid h-12 w-12 place-items-center rounded-lg border border-gold/25 bg-gold/8 text-gold">
      ...
      <section className="py-20">
        <div className="container-px mx-auto max-w-4xl text-center">
          <Reveal>
            <h2 className="font-serif text-3xl md:text-4xl">Let's build something measurable together.</h2>
